...
uint8_t synthSampler_Notes_noteOn(synthSampler_Notes__ctx_type_0 &_ctx, int note, int velocity, int channel){
   ...
   if(_ctx.allowDuplicates || isNew){
      if(bool_not(_ctx.poly)){
         if(bool_not(isNew)){
            synthSampler_Notes_noteOff(_ctx,note,channel);
         }
         _ctx.nb_notes = (1 + _ctx.nb_notes);
...
void synthSampler_Sampler__ctx_type_0_init(synthSampler_Sampler__ctx_type_0 &_output_){
   synthSampler_Sampler__ctx_type_0 &_ctx = _output_;
...
fix16_t synthSampler_Sampler_process(synthSampler_Sampler__ctx_type_0 &_ctx){
   ...
   if(_ctx.state > 0){
      ...
      else
      {
         ...
         {
            value = fix_mul(_ctx.level,(synthSampler_SampleWrapper_getSample(idx) + fix_mul((_ctx.pos % 0x10000 /* 1.000000 */),(synthSampler_SampleWrapper_getSample((1 + idx)) + (- synthSampler_SampleWrapper_getSample(idx))))));
         }
      }
      if(_ctx.quickKill && ((_ctx.level <= 0x0 /* 0.000000 */) || (_ctx.state <= 0))){
         ...
         _ctx.pos = 0x0 /* 0.000000 */;
         _ctx.state = 1;
         synthSampler_Sampler_setNote(_ctx,_ctx.nextNote);
         synthSampler_Sampler_setLevel(_ctx,synthSampler_Util_velocityToLevel(_ctx.nextVelocity));
      }
   }
   return value;
}
   ...
   while(i < nb){
      if(_ctx.state > 0){
         ...
         else
         {
            ...
            {
               oBuffer[i] = fix_mul(_ctx.level,(synthSampler_SampleWrapper_getSample(idx) + fix_mul((_ctx.pos % 0x10000 /* 1.000000 */),(synthSampler_SampleWrapper_getSample((1 + idx)) + (- synthSampler_SampleWrapper_getSample(idx))))));
            }
         }
         if(_ctx.quickKill && ((_ctx.level <= 0x0 /* 0.000000 */) || (_ctx.state <= 0))){
            _ctx.quickKill = false;
...
void synthSampler_Sampler_updateCrossFade(synthSampler_Sampler__ctx_type_0 &_ctx){
   if((_ctx.loopS > (128 / 2)) && ((_ctx.size + (- _ctx.loopE)) > (1 + (128 / 2)))){
      ...
      while(i < 128){
         ...
         _ctx.buffer_cross[i] = (fix_mul(k,synthSampler_SampleWrapper_getSample((i + idxS))) + fix_mul(synthSampler_SampleWrapper_getSample((i + idxE)),(0x10000 /* 1.000000 */ + (- k))));
         i = (1 + i);
      }
   }
   else
...
uint8_t synthSampler_Sampler_noteOn(synthSampler_Sampler__ctx_type_0 &_ctx, int note, int velocity, int channel){
   note = int_clip(note,0,127);
   uint8_t isNew;
   isNew = synthSampler_Notes_noteOn(_ctx.playingnotes,note,velocity,channel);
   _ctx.gate = true;
   if(_ctx.sustain){
      _ctx.sustaining = true;
   }
...
void synthSampler_Sampler_default(synthSampler_Sampler__ctx_type_0 &_ctx){
   _ctx.size = synthSampler_SampleWrapper_getSampleSize();
   synthSampler_Buffer_buffer(_ctx.buffer_cross);
   _ctx.crossfade = false;
   ...
   if(true){
      ...
      if((lS >= 0) && (lE < _ctx.size) && (lE >= lS)){
         ...
         synthSampler_Sampler_setLoopStart(_ctx,lS);
         synthSampler_Sampler_setLoopEnd(_ctx,lE);
      }
   }
   synthSampler_Buffer_buffer(_ctx.buffer_o);
   ...
   synthSampler_Notes_setAllowDuplicates(_ctx.playingnotes,true);
...
void synthSampler_Poly__ctx_type_0_init(synthSampler_Poly__ctx_type_0 &_output_){
   ...
   synthSampler_Sampler__ctx_type_0_init(_ctx.voice3);
   synthSampler_Sampler__ctx_type_0_init(_ctx.voice2);
   synthSampler_Sampler__ctx_type_0_init(_ctx.voice1);
   synthSampler_Sampler__ctx_type_0_init(_ctx.voice0);
...
void synthSampler_Voice__ctx_type_0_init(synthSampler_Voice__ctx_type_0 &_output_){
   ...
   synthSampler_Notes__ctx_type_0_init(_ctx.voiceinsactive);
   _ctx.reuse = false;
...
void synthSampler_Voice_noteOn(synthSampler_Voice__ctx_type_0 &_ctx, int note, int velocity, int channel){
   ...
   if((v > 0) && (v <= _ctx.number_voices)){
      if(bool_not((synthSampler_Notes_noteOff(_ctx.voicesactive,((-1) + v),0) && synthSampler_Notes_noteOn(_ctx.voicesinactive,((-1) + v),127,0) && synthSampler_Notes_noteOff(_ctx.voicesinactive,((-1) + v),0) && synthSampler_Notes_noteOn(_ctx.voicesactive,((-1) + v),127,0)))){
         _ctx.notes[note] = 0;
         _ctx.voices[((-1) + v)] = 0;
         v = 0;
      }
   ...
   else
   {
      if(_ctx.reuse && (v < 0) && ((- v) <= _ctx.number_voices) && (_ctx.voices[((-1) + (- v))] == (- (1 + note))) && (_ctx.notes[note] == v)){
         v = (- v);
         if(bool_not((synthSampler_Notes_noteOff(_ctx.voicesinactive,((-1) + v),0) && synthSampler_Notes_noteOn(_ctx.voicesactive,((-1) + v),127,0)))){
            _ctx.notes[note] = 0;
            _ctx.voices[((-1) + v)] = 0;
            v = 0;
         }
...
void synthSampler_Voice_default(synthSampler_Voice__ctx_type_0 &_ctx){
   ...
   synthSampler_Buffer_buffer(_ctx.buffer_v0);
   synthSampler_Buffer_buffer(_ctx.buffer_v1);
   synthSampler_Buffer_buffer(_ctx.buffer_v2);
   synthSampler_Buffer_buffer(_ctx.buffer_v3);
   if(_ctx.number_voices == 0){
