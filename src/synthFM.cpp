...
fix16_t synthFM_Wavetable_getSample(int wavetableIdx, int index){
   ...
   switch(wavetableIdx) {
      ...
      break;
      case 9:
         sample = synthFM_Wavetable_noise_wave(0,index);
      break;
...
void synthFM_Wavetable_morphTo(fix16_t wavetableIdx, fix16_t phase, fix16_t (&buffer)[4096]){
   wavetableIdx = fix_clip(wavetableIdx,0x0 /* 0.000000 */,0xa0000 /* 10.000000 */);
   int phase_shift;
...
void synthFM_Voice_default(synthFM_Voice__ctx_type_0 &_ctx){
   ...
   synthFM_Buffer_buffer(_ctx.buffer_v1);
   synthFM_Buffer_buffer(_ctx.buffer_v2);
   synthFM_Buffer_buffer(_ctx.buffer_v3);
   if(_ctx.number_voices == 0){
