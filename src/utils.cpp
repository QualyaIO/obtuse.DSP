...
void utils_Gate__ctx_type_1_init(utils_Gate__ctx_type_1 &_output_){
   ...
   utils_Gate__ctx_type_0_init(_ctx.notesOn);
   utils_Gate__ctx_type_0_init(_ctx.notesOff);
   utils_Gate__ctx_type_0_init(_ctx.notesActive);
   fix_init_array(128,0x0 /* 0.000000 */,_ctx.notes);
...
void utils_Gate_process(utils_Gate__ctx_type_1 &_ctx){
   ...
   while(bool_not(found) && (i < nbActive)){
      ...
      if((note >= 0) && (note < 128)){
         if((_ctx.time >= _ctx.notes[note]) && ((_ctx.time + (- _ctx.notes[note])) < _ctx.gateDuration)){
            found = true;
         }
      }
      if(bool_not(found)){
...
uint8_t utils_Gate_noteOn(utils_Gate__ctx_type_1 &_ctx, int note, int velocity, int channel){
   ...
   uint8_t newNote;
   newNote = (_ctx.notes[note] == 0x0 /* 0.000000 */);
   if(newNote){
