...
void synthSamplerCello_Sampler_noteOn(synthSamplerCello_Sampler__ctx_type_0 &_ctx, int note, int velocity, int channel){
   note = int_clip(note,0,127);
   if(synthSamplerCello_Notes_noteOn(_ctx.playingnotes,note,velocity,channel)){
...
void synthSamplerCello_Voice_noteOn(synthSamplerCello_Voice__ctx_type_0 &_ctx, int note, int velocity, int channel){
   ...
   if(_ctx.notes[note] <= 0){
      ...
      v = synthSamplerCello_Notes_firstNote(_ctx.voicesinactive);
      if((v > 0) && (v <= _ctx.number_voices)){
         if(synthSamplerCello_Notes_noteOff(_ctx.voicesinactive,((-1) + v),0) && synthSamplerCello_Notes_noteOn(_ctx.voicesactive,((-1) + v),127,0)){
