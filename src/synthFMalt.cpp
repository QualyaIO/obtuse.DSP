...
void synthFMalt_Wavetable_getRandomMorph(fix16_t (&oBuff)[4096]){
   int basetable;
   basetable = 0;
   if(8 > 1){
      basetable = (irandom() % 7);
...
fix16_t synthFMalt_Wavetable_getRandRandomMorph(fix16_t (&oBuff)[4096]){
   synthFMalt_Wavetable_getRandomMorph(oBuff);
   int idx;
   idx = 0;
   if(4096 > 0){
      idx = (irandom() % 4096);
