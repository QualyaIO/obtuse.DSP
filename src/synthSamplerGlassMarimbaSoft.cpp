...
void synthSamplerGlassMarimbaSoft_Notes__ctx_type_0_init(synthSamplerGlassMarimbaSoft_Notes__ctx_type_0 &_output_){
   synthSamplerGlassMarimbaSoft_Notes__ctx_type_0 &_ctx = _output_;
   _ctx.poly = false;
   int_init_array(128,0,_ctx.notes);
   _ctx.nb_notes = 0;
   int_init_array(128,0,_ctx.last_notes);
   _ctx.allowDuplicates = false;
   synthSamplerGlassMarimbaSoft_Notes_default(_ctx);
...
uint8_t synthSamplerGlassMarimbaSoft_Notes_noteOn(synthSamplerGlassMarimbaSoft_Notes__ctx_type_0 &_ctx, int note, int velocity, int channel){
   ...
   if(_ctx.allowDuplicates || isNew){
      if(bool_not(_ctx.poly)){
         if(bool_not(isNew)){
            synthSamplerGlassMarimbaSoft_Notes_noteOff(_ctx,note,channel);
         }
         _ctx.nb_notes = (1 + _ctx.nb_notes);
      ...
      {
         int last_note;
         last_note = synthSamplerGlassMarimbaSoft_Notes_lastNote(_ctx);
         if(last_note > 0){
...
void synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0_init(synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_output_){
   synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_ctx = _output_;
   ...
   synthSamplerGlassMarimbaSoft_Notes__ctx_type_0_init(_ctx.playingnotes);
   ...
   synthSamplerGlassMarimbaSoft_Sampler_default(_ctx);
...
fix16_t synthSamplerGlassMarimbaSoft_Sampler_process(synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_ctx){
   ...
   if(_ctx.state > 0){
      ...
      else
      {
         ...
         {
            value = fix_mul(_ctx.level,(synthSamplerGlassMarimbaSoft_SampleWrapper_getSample(idx) + fix_mul((_ctx.pos % 0x10000 /* 1.000000 */),(synthSamplerGlassMarimbaSoft_SampleWrapper_getSample((1 + idx)) + (- synthSamplerGlassMarimbaSoft_SampleWrapper_getSample(idx))))));
         }
      }
      if(_ctx.quickKill && ((_ctx.level <= 0x0 /* 0.000000 */) || (_ctx.state <= 0))){
         ...
         _ctx.pos = 0x0 /* 0.000000 */;
         _ctx.state = 1;
         synthSamplerGlassMarimbaSoft_Sampler_setNote(_ctx,_ctx.nextNote);
         synthSamplerGlassMarimbaSoft_Sampler_setLevel(_ctx,synthSamplerGlassMarimbaSoft_Util_velocityToLevel(_ctx.nextVelocity));
      }
   }
   return value;
}

void synthSamplerGlassMarimbaSoft_Sampler_process_bufferTo(synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_ctx, int nb, fix16_t (&oBuffer)[128]){
   ...
   while(i < nb){
      if(_ctx.state > 0){
         ...
         else
         {
            ...
            {
               oBuffer[i] = fix_mul(_ctx.level,(synthSamplerGlassMarimbaSoft_SampleWrapper_getSample(idx) + fix_mul((_ctx.pos % 0x10000 /* 1.000000 */),(synthSamplerGlassMarimbaSoft_SampleWrapper_getSample((1 + idx)) + (- synthSamplerGlassMarimbaSoft_SampleWrapper_getSample(idx))))));
            }
         }
         if(_ctx.quickKill && ((_ctx.level <= 0x0 /* 0.000000 */) || (_ctx.state <= 0))){
            _ctx.quickKill = false;
...
void synthSamplerGlassMarimbaSoft_Sampler_updateCrossFade(synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_ctx){
   if((_ctx.loopS > (128 / 2)) && ((_ctx.size + (- _ctx.loopE)) > (1 + (128 / 2)))){
      ...
      while(i < 128){
         ...
         _ctx.buffer_cross[i] = (fix_mul(k,synthSamplerGlassMarimbaSoft_SampleWrapper_getSample((i + idxS))) + fix_mul(synthSamplerGlassMarimbaSoft_SampleWrapper_getSample((i + idxE)),(0x10000 /* 1.000000 */ + (- k))));
         i = (1 + i);
      }
   }
   else
...
uint8_t synthSamplerGlassMarimbaSoft_Sampler_noteOn(synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_ctx, int note, int velocity, int channel){
   note = int_clip(note,0,127);
   uint8_t isNew;
   isNew = synthSamplerGlassMarimbaSoft_Notes_noteOn(_ctx.playingnotes,note,velocity,channel);
   _ctx.gate = true;
   if(_ctx.sustain){
      _ctx.sustaining = true;
   }
   if(_ctx.state <= 0){
      ...
      _ctx.pos = 0x0 /* 0.000000 */;
      _ctx.state = 1;
      synthSamplerGlassMarimbaSoft_Sampler_setNote(_ctx,note);
      synthSamplerGlassMarimbaSoft_Sampler_setLevel(_ctx,synthSamplerGlassMarimbaSoft_Util_velocityToLevel(velocity));
   }
   else
   {
      _ctx.quickKill = true;
...
void synthSamplerGlassMarimbaSoft_Sampler_noteOff(synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_ctx, int note, int channel){
   note = int_clip(note,0,127);
   if(synthSamplerGlassMarimbaSoft_Notes_noteOff(_ctx.playingnotes,note,channel)){
      if(synthSamplerGlassMarimbaSoft_Notes_nbNotes(_ctx.playingnotes) > 0){
         int last_played;
         last_played = synthSamplerGlassMarimbaSoft_Notes_lastNote(_ctx.playingnotes);
         if((last_played > 0) && (last_played <= 128)){
            ...
            {
               synthSamplerGlassMarimbaSoft_Sampler_setNote(_ctx,((-1) + last_played));
            }
         }
      }
      else
...
void synthSamplerGlassMarimbaSoft_Sampler_default(synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0 &_ctx){
   _ctx.size = synthSamplerGlassMarimbaSoft_SampleWrapper_getSampleSize();
   synthSamplerGlassMarimbaSoft_Buffer_buffer(_ctx.buffer_cross);
   _ctx.crossfade = false;
   ...
   if(false){
      ...
      if((lS >= 0) && (lE < _ctx.size) && (lE >= lS)){
         synthSamplerGlassMarimbaSoft_Sampler_setLoop(_ctx,true);
         synthSamplerGlassMarimbaSoft_Sampler_setLoopStart(_ctx,lS);
         synthSamplerGlassMarimbaSoft_Sampler_setLoopEnd(_ctx,lE);
      }
   }
   synthSamplerGlassMarimbaSoft_Buffer_buffer(_ctx.buffer_o);
   synthSamplerGlassMarimbaSoft_Sampler_setLevel(_ctx,0x10000 /* 1.000000 */);
   synthSamplerGlassMarimbaSoft_Sampler_setSamplerate(_ctx,0x2c1999 /* 44.100000 */);
   synthSamplerGlassMarimbaSoft_Sampler_setNote(_ctx,69);
   synthSamplerGlassMarimbaSoft_Notes_default(_ctx.playingnotes);
   synthSamplerGlassMarimbaSoft_Notes_setAllowDuplicates(_ctx.playingnotes,true);
   synthSamplerGlassMarimbaSoft_Sampler_setPoly(_ctx,false);
}

void synthSamplerGlassMarimbaSoft_Poly__ctx_type_0_init(synthSamplerGlassMarimbaSoft_Poly__ctx_type_0 &_output_){
   synthSamplerGlassMarimbaSoft_Poly__ctx_type_0 &_ctx = _output_;
   synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0_init(_ctx.voice3);
   synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0_init(_ctx.voice2);
   synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0_init(_ctx.voice1);
   synthSamplerGlassMarimbaSoft_Sampler__ctx_type_0_init(_ctx.voice0);
   _ctx.should_leftovers = false;
   synthSamplerGlassMarimbaSoft_Poly_default(_ctx);
...
fix16_t synthSamplerGlassMarimbaSoft_Poly_getSample(synthSamplerGlassMarimbaSoft_Poly__ctx_type_0 &_ctx, int voice){
   switch(voice) {
      case 0:
         return synthSamplerGlassMarimbaSoft_Sampler_process(_ctx.voice0);
      break;
      case 1:
         return synthSamplerGlassMarimbaSoft_Sampler_process(_ctx.voice1);
      break;
      case 2:
         return synthSamplerGlassMarimbaSoft_Sampler_process(_ctx.voice2);
      break;
      case 3:
         return synthSamplerGlassMarimbaSoft_Sampler_process(_ctx.voice3);
      break;
...
void synthSamplerGlassMarimbaSoft_Poly_default(synthSamplerGlassMarimbaSoft_Poly__ctx_type_0 &_ctx){
   _ctx.should_leftovers = false;
   synthSamplerGlassMarimbaSoft_Sampler_default(_ctx.voice0);
   synthSamplerGlassMarimbaSoft_Sampler_setPoly(_ctx.voice0,true);
   synthSamplerGlassMarimbaSoft_Sampler_default(_ctx.voice1);
   synthSamplerGlassMarimbaSoft_Sampler_setPoly(_ctx.voice1,true);
   synthSamplerGlassMarimbaSoft_Sampler_default(_ctx.voice2);
   synthSamplerGlassMarimbaSoft_Sampler_setPoly(_ctx.voice2,true);
   synthSamplerGlassMarimbaSoft_Sampler_default(_ctx.voice3);
   synthSamplerGlassMarimbaSoft_Sampler_setPoly(_ctx.voice3,true);
}

void synthSamplerGlassMarimbaSoft_Voice__ctx_type_0_init(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_output_){
   synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx = _output_;
   synthSamplerGlassMarimbaSoft_Notes__ctx_type_0_init(_ctx.voicesinactive);
   synthSamplerGlassMarimbaSoft_Notes__ctx_type_0_init(_ctx.voicesactive);
   ...
   synthSamplerGlassMarimbaSoft_Voice_default(_ctx);
...
fix16_t synthSamplerGlassMarimbaSoft_Voice_process(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx){
   ...
   while(i < _ctx.number_voices){
      _ctx.last_values[i] = synthSamplerGlassMarimbaSoft_Poly_getSample(_ctx.poly,i);
...
void synthSamplerGlassMarimbaSoft_Voice_process_bufferTo(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx, int nb, fix16_t (&oBuffer)[128]){
   nb = int_clip(nb,0,128);
   if(nb == 0){
      nb = 128;
   }
   ...
   if(v < _ctx.number_voices){
      synthSamplerGlassMarimbaSoft_Poly_runVoice(_ctx.poly,v,nb,_ctx.buffer_v0);
      i = 0;
      while(i < nb){
...
void synthSamplerGlassMarimbaSoft_Voice_process_bufferTo_alt(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx, int nb, fix16_t (&oBuffer)[128]){
   nb = int_clip(nb,0,128);
   if(nb == 0){
      nb = 128;
   }
   synthSamplerGlassMarimbaSoft_Poly_runVoice(_ctx.poly,0,nb,_ctx.buffer_v0);
   synthSamplerGlassMarimbaSoft_Poly_runVoice(_ctx.poly,1,nb,_ctx.buffer_v1);
   synthSamplerGlassMarimbaSoft_Poly_runVoice(_ctx.poly,2,nb,_ctx.buffer_v2);
   synthSamplerGlassMarimbaSoft_Poly_runVoice(_ctx.poly,3,nb,_ctx.buffer_v3);
   if(nb > 0){
...
void synthSamplerGlassMarimbaSoft_Voice_noteOn(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx, int note, int velocity, int channel){
   ...
   if(v > 0){
      if(synthSamplerGlassMarimbaSoft_Poly_shouldLeftOvers(_ctx.poly)){
         _ctx.leftovers = (_ctx.leftovers + _ctx.last_values[((-1) + v)]);
      }
      synthSamplerGlassMarimbaSoft_Poly_sendNoteOn(_ctx.poly,((-1) + v),note,velocity,channel);
      _ctx.notes[note] = v;
...
void synthSamplerGlassMarimbaSoft_Voice_setNbVoices(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx, int nbvoices){
   ...
   int i;
   i = synthSamplerGlassMarimbaSoft_Notes_nbNotes(_ctx.voicesactive);
   while((i > nbvoices) && (i > 0)){
      synthSamplerGlassMarimbaSoft_Voice_noteOff(_ctx,_ctx.voices[((-1) + i)],0);
      synthSamplerGlassMarimbaSoft_Notes_noteOff(_ctx.voicesinactive,((-1) + i),0);
      i = ((-1) + i);
   }
   i = synthSamplerGlassMarimbaSoft_Notes_nbNotes(_ctx.voicesinactive);
   if(i < nbvoices){
      while(i < nbvoices){
         synthSamplerGlassMarimbaSoft_Notes_noteOn(_ctx.voicesinactive,i,127,0);
         i = (1 + i);
      }
   }
   else
   {
      while(i >= nbvoices){
         synthSamplerGlassMarimbaSoft_Notes_noteOff(_ctx.voicesinactive,i,0);
         i = ((-1) + i);
      }
   }
...
void synthSamplerGlassMarimbaSoft_Voice_setSamplerate(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx, fix16_t newFs){
   ...
   synthSamplerGlassMarimbaSoft_Poly_setSamplerate(_ctx.poly,_ctx.fs);
   fix16_t log_base;
...
void synthSamplerGlassMarimbaSoft_Voice_default(synthSamplerGlassMarimbaSoft_Voice__ctx_type_0 &_ctx){
   synthSamplerGlassMarimbaSoft_Poly_default(_ctx.poly);
   synthSamplerGlassMarimbaSoft_Buffer_buffer(_ctx.buffer_v0);
   synthSamplerGlassMarimbaSoft_Buffer_buffer(_ctx.buffer_v1);
   synthSamplerGlassMarimbaSoft_Buffer_buffer(_ctx.buffer_v2);
   synthSamplerGlassMarimbaSoft_Buffer_buffer(_ctx.buffer_v3);
   if(_ctx.number_voices == 0){
      _ctx.number_voices = 4;
   }
   synthSamplerGlassMarimbaSoft_Voice_setNbVoices(_ctx,_ctx.number_voices);
   synthSamplerGlassMarimbaSoft_Notes_default(_ctx.voicesactive);
   synthSamplerGlassMarimbaSoft_Notes_setPoly(_ctx.voicesactive,false);
   synthSamplerGlassMarimbaSoft_Notes_setAllowDuplicates(_ctx.voicesactive,false);
   synthSamplerGlassMarimbaSoft_Notes_default(_ctx.voicesinactive);
   synthSamplerGlassMarimbaSoft_Notes_setPoly(_ctx.voicesinactive,false);
...
void synthSamplerGlassMarimbaSoft_Voice__ctx_type_1_init(synthSamplerGlassMarimbaSoft_Voice__ctx_type_1 &_output_){
   synthSamplerGlassMarimbaSoft_Voice__ctx_type_1 &_ctx = _output_;
   synthSamplerGlassMarimbaSoft_Voice__ctx_type_0_init(_ctx._inst275);
   synthSamplerGlassMarimbaSoft_Voice__ctx_type_0_init(_ctx._inst1b9);
